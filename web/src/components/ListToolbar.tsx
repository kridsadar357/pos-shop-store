...
function FilterPopover({ children, count, onReset }: { children: ReactNode; count: number; onReset?: () => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <i className="fa-solid fa-sliders" /> {th.tbFilter}
      ...
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-[340px] rounded-2xl bg-white p-4 shadow-pop ring-1 ring-slate-200">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm font-bold text-ink-900">{th.tbAdvFilter}</span>
            {onReset && <button className="text-xs font-semibold text-rose-500 hover:underline" onClick={onReset}>{th.tbClearFilter}</button>}
          </div>
          <div className="space-y-3">{children}</div>
          <button className="btn-primary mt-4 w-full" onClick={() => setOpen(false)}>{th.tbApplyFilter}</button>
        </div>
      )}
    </div>
  );
...
function ExportMenu({ exports }: { exports: ExportHandlers }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <i className="fa-solid fa-file-export" /> {th.tbExport} <i className="fa-solid fa-chevron-down text-[10px] text-slate-400" />
      </button>
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-48 overflow-hidden rounded-xl bg-white py-1 shadow-pop ring-1 ring-slate-200">
          ...
          {item('fa-solid fa-file-zipper', th.tbImagesZip, 'text-amber-600', exports.zip)}
        </div>
      )}
    </div>
  );
