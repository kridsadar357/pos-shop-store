...
export default function Movements() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Stock Ledger"
        subtitle="Every stock movement — full backtrack audit trail"
        icon="↺"
        actions={
      ...
      <div className="flex flex-wrap gap-2">
        <input className="input max-w-xs" placeholder="Filter by product…" value={q} onChange={(e) => setQ(e.target.value)} />
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-400">
            <tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Product</th><th className="px-4 py-3">Type</th><th className="px-4 py-3 text-right">Change</th><th className="px-4 py-3 text-right">Balance</th><th className="px-4 py-3">Ref / Note</th><th className="px-4 py-3">User</th></tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {filtered.map((r) => (
              <tr key={r.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5"><span className={`chip ${COLOR[r.type] ?? 'bg-slate-100 text-slate-600'}`}>{r.type}</span></td>
            ...
            {filtered.length === 0 && <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-400">No movements.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
